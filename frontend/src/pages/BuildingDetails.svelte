...
<Modal bind:open={modalOpen} title={editingId ? $locale.floors.edit : $locale.floors.add} description={$locale.floors.description} busy={saving} icon="bi-layers" closeLabel={$locale.floors.cancel} on:close={closeModal}>
  <form id="floor-form" on:submit|preventDefault={saveFloor} novalidate>{#if lastSavedFloorNumber !== null}<div class="alert alert-success" role="status">{translate('floors.savedAnother', { number: lastSavedFloorNumber })}</div>{/if}{#if modalError}<div class="alert alert-danger" role="alert">{modalError}</div>{/if}<div class="field"><label class="form-label" for="floor-number">{$locale.floors.floorNumber}</label><div class="field-control"><i class="bi bi-123" aria-hidden="true"></i><input class:is-invalid={formErrors.floorNumber} class="form-control" id="floor-number" type="text" maxlength="32" bind:value={form.floorNumber} /></div>{#if formErrors.floorNumber}<div class="invalid-feedback">{formErrors.floorNumber}</div>{/if}</div><div class="field"><label class="form-label" for="floor-name">{$locale.floors.name}</label><div class="field-control"><i class="bi bi-tag" aria-hidden="true"></i><input class:is-invalid={formErrors.name} class="form-control" id="floor-name" bind:value={form.name} /></div>{#if formErrors.name}<div class="invalid-feedback">{formErrors.name}</div>{/if}</div></form>
