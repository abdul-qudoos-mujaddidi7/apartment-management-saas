<script>
  ...
  const emptyForm = () => ({ buildingId: '', floorId: '', apartmentId: '', leaseId: '', invoiceDate: new Date().toISOString().slice(0, 10), dueDate: '', notes: '', items: [newItem()] });
  ...
  function openCreate() { editingId = null; floors = []; apartments = []; leases = []; form = emptyForm(); modalError = ''; formErrors = {}; modalOpen = true; }
  ...
  function leaseChanged() { const lease = selectedLease(); if (!lease || form.items.length !== 1 || form.items[0].type !== 'RENT') return; form = { ...form, items: [{ ...form.items[0], unitPrice: lease.monthlyRent }] }; }
  ...
  async function openEdit(invoice) {
    try {
      ...
      form = { buildingId: floor.building.id, floorId: '', apartmentId: '', leaseId: '', invoiceDate: fi.invoiceDate.slice(0, 10), dueDate: fi.dueDate ? fi.dueDate.slice(0, 10) : '', notes: fi.notes || '', items: fi.items.map((item) => ({ type: item.type, description: item.description, quantity: item.quantity, unitPrice: item.unitPrice })) };
  ...
  async function saveInvoice() {
    ...
    const payload = { invoiceDate: form.invoiceDate, dueDate: form.dueDate || null, notes: form.notes.trim() || null, items: form.items.map((item) => ({ type: item.type, description: item.description.trim(), quantity: Number(item.quantity), unitPrice: Number(item.unitPrice) })) };
    try {
