...
<!-- Quick Reading Modal -->
<Modal bind:open={readingModalOpen} title={$locale.meterReadings.quickAdd} description={$locale.meterReadings.description} busy={readingSaving} icon="bi-speedometer" closeLabel={$locale.meterReadings.cancel} on:close={() => closeReadingModal()}>
  {#if selectedMeter}
...
<Modal bind:open={modalOpen} title={editingId ? $locale.meters.edit : $locale.meters.add} description={$locale.meters.description} busy={saving} size="modal-lg" icon="bi-speedometer2" closeLabel={$locale.meters.cancel} on:close={closeModal}>
